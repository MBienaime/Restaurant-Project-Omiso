...
const Menus = () => {
  ...
  function handlesubmitMenu(e) {
    ...
    axios
      .post("https://omiso.com/menu/", formData, {
  ...
  function handleRemoveMenu(id) {
    const token = window.localStorage.getItem("UserTokenOmiso");
    axios
      .delete(`https://omiso.com/menu/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(() => {
        getApiData();
    ...
    <div className="sectionAdminMenu">
      ...
      <form className="ResultSelectAdminMenu">
        ...
        <input
          ...
          maxLength="2"
          onChange={(e) => handleInputChange(e)}
          value={AddDataMenu.prix}
        />
        <label htmlFor="category">Catégorie:</label>
        <select
          className="ResultSelectAdminMenu_select"
          name="category"
          id="category"
          onChange={(e) => handleInputChange(e)}
          value={AddDataMenu.category}
        >
          <option className="ResultSelectAdminMenu_select" value="entree">
            Entree
          </option>
          <option className="ResultSelectAdminMenu_select" value="plat">
            Plat
          </option>
          <option className="ResultSelectAdminMenu_select" value="dessert">
            Dessert
          </option>
          <option className="ResultSelectAdminMenu_select" value="boisson">
            Boisson
          </option>
        </select>
        ...
        <img
          className="ResultSelectAdminMenu_imagesViews_image"
          src={Image.preview}
        />

        <button
          className="ResultSelectAdminMenu_imagesViews_button"
          onClick={(e) => handlesubmitMenu(e)}
        >
          ajouter
        </button>
      </form>
