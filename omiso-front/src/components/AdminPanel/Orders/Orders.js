  ...
  return (
    <div className="sectionAdminMenu">
      <table className="sectionAdminMenu_table">
          ...
          <tr>
            <td>
              ...
              {viewsArchive ? (
                <button
                  type="button"
                  onClick={() => setViewsArchive(!viewsArchive)}
                >
                  Commande Archivé
                </button>
              ) : (
        ...
        <tbody>
          {dataOrder
            .filter((ele) => ele.status == viewsArchive)
            .map((e) => (
              <tr
                key={uuidv4()}
                onClick={() => setDetailOrder(e._id)}
                className={e._id == detailOrder ? "selectOrder" : ""}
              >
                ...
                <td className="btn">
                  ...
                  <button className=" btn-fa">
                    {e.status ? (
                      <FaToggleOn onClick={() => toggleArchive(e._id)} />
                    ) : (
                      <FaToggleOff onClick={() => toggleArchive(e._id)} />
                    )}
                  </button>
                </td>
              </tr>
      ...
      <div className="OrderDetail">
        ...
        {filterCategory(dataOrder, detailOrder).map((el) => (
          <div className="OrderDetail_client" key={uuidv4()}>
            ...
            <button type="button">
              {el.status ? (
                <FaToggleOn onClick={() => toggleArchive(el._id)} />
              ) : (
                <FaToggleOff onClick={() => toggleArchive(el._id)} />
              )}
            </button>
