...
const SectionMenu = ({ addOrder }) => {
  ...
  const getApiData = () => {
    ...
    axios
      .get(url)
      .then((resp) => {
        const addquantity = resp.data.menuItems.map((e) => ({
          ...e,
          quantity: 0,
        }));
        setData(addquantity);
  ...
  return (
    <>
      <div className="sectionMenus" id="Menu">
        <ul className="sectionMenusCarteMenu">
          <li className="buttonMenu" onClick={() => setCategory("entree")}>
            Entree
          </li>
          <li className="buttonMenu" onClick={() => setCategory("plat")}>
            Plats
          </li>
          <li className="buttonMenu" onClick={() => setCategory("dessert")}>
            Dessert
          </li>
          <li className="buttonMenu" onClick={() => setCategory("boisson")}>
            Boisson
          </li>
        </ul>
        ...
        <div className="sectionMenuCarte">
          {filterCategory(data, category).map((data) => (
            <ItemMenu data={data} addOrder={addOrder} key={uuidv4()} />
          ))}
        </div>
        <Footer />
      </div>
